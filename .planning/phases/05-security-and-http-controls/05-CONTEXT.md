...
- `VIGILO_COMPATIBILITY.md` lines 94-112 — original auth/rate-limit/request-size target. User decisions in this CONTEXT.md refine that target: ingress gets a separate token, `/v1/readyz` is public by default but configurable, `/v1/metrics` exposure is configurable, and public paths use named exposure flags.
