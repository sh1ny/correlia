...
- **D-18:** Required fields: `decision_kind` (problem|recovery|noop), `incident_effect` (none|inserted|updated|resolved|affected_set_shrunk), `no_dispatch_reason` (null|below_threshold|replay|already_notified), `rule_name`, `group_key`, `incident_ids` (max 20), `affected_incident_count` (int), `incident_ids_truncated` (bool), `counted_count`, `threshold_count`, `threshold_crossed`, `replay`, `first_threshold_transition`, `notification_intent` (dispatch_planned|no_dispatch), `recovery_resolution`, `affected_object_removed`.
