...
- `compatibility research (removed for privacy)` §1 — original incident API compatibility target. **Superseded:** Phase 6 implements parity on canonical `/v1/incidents` only; no `/api/v1/incidents` facade or down-projection.
