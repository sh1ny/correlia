...
- `compatibility research (removed for privacy)` §3 (lines 73-92) — original migration command target and constraints (single `--rules`/`--topology`/`--plugins` inputs, plaintext credential prohibition, fail on unsupported fields).
