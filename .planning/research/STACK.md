...
- No `litellm` in core dependencies. LLM enrichment/decision plugins should be optional plugin-level dependencies only, disabled by default, per `VIGILO_COMPATIBILITY.md` rules.
...
Use FastAPI `HTTPBearer` for static Bearer-token auth, but **do not** copy VDE's `DEV_MODE` bypass or random Easter-egg message. Per `VIGILO_COMPATIBILITY.md`:
...
- `app/api/routers/*.py` — apply auth dependency to `/v1/incidents`, `/v1/rules`, `/v1/topology`, `/v1/plugins`, `/v1/metrics` as specified in `VIGILO_COMPATIBILITY.md`.
...
`VIGILO_COMPATIBILITY.md` mandates these gates run from the Correlia root:
...
Per `VIGILO_COMPATIBILITY.md`, add Prometheus counters only for new compatibility behavior:
...
Correlia uses Alembic; VDE does not. Any schema additions (e.g., `incident_events` audit table from `VIGILO_COMPATIBILITY.md`) must be delivered as Alembic revisions, not raw SQL in docker-entrypoint.
...
- `VIGILO_COMPATIBILITY.md` and `CONFIGURATION.md` in Correlia root.
