...
**Source documents:** `compatibility research (removed for privacy)`, `CONFIGURATION.md`.
...
- **No plaintext in generated YAML.** This is a hard requirement from `compatibility research (removed for privacy)`.
...
- A `scripts/migrate_vigilo_config.py` command must be added, matching the inputs/outputs described in `compatibility research (removed for privacy)`.
