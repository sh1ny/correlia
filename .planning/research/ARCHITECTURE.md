...
**Sources inspected:** Correlia source + `VIGILO_COMPATIBILITY.md` + `CONFIGURATION.md` + VDE (`/home/bgshi/Development/python/vde-event-aggregation`) source.
...
Per `VIGILO_COMPATIBILITY.md`, add an append-only `incident_events` table (do not reuse VDE's `raw_events` name):
