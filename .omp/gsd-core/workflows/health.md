...
visible in your runtime (OMP's `/context` slash command output, or the
model's own session telemetry). If the runtime exposes neither, prompt the user
once via Ask for both numbers.

**TEXT_MODE fallback:** when `text_mode` is true (config or `--text` flag) the
runtime is non-Claude (Codex, Gemini, etc.) and `Ask` is not
...
<stale_task_cleanup>
**Windows-specific:** Check for stale OMP task directories that accumulate on crash/freeze.
...
TASKS_DIR="/home/bgshi/Development/python/correlia/.omp/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /home/bgshi/Development/python/correlia/.omp/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /home/bgshi/Development/python/correlia/.omp/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
