...
- Ask inline (freeform, NOT Ask): "What do you want to build next?"
- Wait for their response, then use Ask to probe specifics
- If user selects "Other" at any point to provide freeform input, ask follow-up as plain text — not another Ask
...
**If "Adjust":** Ask what needs changing (plain text, NOT Ask). Incorporate changes, re-present the summary. Loop until "Looks good" is selected.
...
Use template: /home/bgshi/Development/python/correlia/.omp/gsd-core/templates/research-project/{FILE}
</output>
...
**Scope each category** via Ask (multiSelect: true, header max 12 chars):
