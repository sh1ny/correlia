...
**GSD Core** (Git. Ship. Done.) creates hierarchical project plans optimized for solo agentic development with OMP.
...
- Generates `./.omp/skills/spike-findings-[project]/` with references and sources
...
- Generates `./.omp/skills/sketch-findings-[project]/` with design decisions, CSS patterns, HTML structures
...
- Saves timestamped note to `.planning/notes/` (or `/home/bgshi/Development/python/correlia/.omp/notes/` globally)
...
- **`/gsd-ultraplan-phase [phase]`** — [BETA] Offload plan phase to OMP's ultraplan cloud; review in browser and import back.
