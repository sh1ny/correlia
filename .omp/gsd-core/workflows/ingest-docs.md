...
If user passed `--mode new` but `.planning/` already exists: display warning and require explicit confirm via `Ask` (approve-revise-abort from `references/gate-prompts.md`) before overwriting.
...
**Text mode:** apply the same `--text`/`text_mode` rule as other workflows — replace `Ask` with a numbered list.

Use `Ask` (approve-revise-abort):
...
For each discovered doc, spawn `gsd-doc-classifier` in parallel. In OMP, issue all Task calls in a single message with multiple tool uses so the harness runs them concurrently. For Copilot / sequential runtimes, fall back to sequential dispatch.
...
Render the report, then ask via Ask (approve-revise-abort):
