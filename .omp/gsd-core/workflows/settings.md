...
Use Ask with current values pre-selected. Questions are grouped into six visual sections; the first question in each section carries the section-denoting `header` field (Ask renders abbreviated section tags for grouping, max 12 chars).
...
```
// Model profile is selected via a two-question split because Ask enforces a
...
Ask([
  ...
  // the Ask call and do not touch the existing workflow.code_review_depth value.
  {
