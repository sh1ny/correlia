...
<required_reading>
@/home/bgshi/Development/python/correlia/.omp/gsd-core/references/domain-probes.md
@/home/bgshi/Development/python/correlia/.omp/gsd-core/references/gate-prompts.md
@/home/bgshi/Development/python/correlia/.omp/gsd-core/references/universal-anti-patterns.md
</required_reading>
...
<answer_validation>
**IMPORTANT: Answer validation** — After every Ask call, if the response is empty/whitespace-only:

- **"Other" with empty text** (the user wants to type freeform): output `"What would you like to discuss?"`, STOP generating, wait for the user's next message, then reflect it back and continue. Do NOT retry Ask or call any tools.
...
**Text mode** (`--text` or `workflow.text_mode: true`): follow `workflows/discuss-phase/modes/text.md` — do not use Ask at all.
</answer_validation>
...
if [ -f "/home/bgshi/Development/python/correlia/.omp/gsd-core/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
- Otherwise, continue. Per-flag overlay reads happen at their relevant steps:
  ...
  - `--text` (or `workflow.text_mode: true`) → Read `workflows/discuss-phase/modes/text.md` before any Ask call.
...
**Otherwise:** Ask (header: "Context"; question: "Phase [X] already has context. What do you want to do?"; options: "Update it" / "View it" / "Skip"). Branch accordingly.
...
**Otherwise:** Ask (header: "Plans exist"; question: "Phase [X] already has {plan_count} plan(s) created without user context. Your decisions here won't affect existing plans unless you replan."; options: "Continue and replan after" / "View existing plans" / "Cancel"). Branch accordingly.
...
```bash
SPIKE_FINDINGS=$(ls ./.omp/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1 || true)
SKETCH_FINDINGS=$(ls ./.omp/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1 || true)
...
**If matches found:** Present each match (title, area, why it matched). Ask (multiSelect) asking which to fold. Folded → `<folded_todos>` for CONTEXT.md `<decisions>`. Reviewed but not folded → `<reviewed_todos>` for CONTEXT.md `<deferred>`.
...
Read `@/home/bgshi/Development/python/correlia/.omp/gsd-core/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
...
```
Apply each entry in `activeHooks` per @/home/bgshi/Development/python/correlia/.omp/gsd-core/references/loop-hook-dispatch.md. Empty list → continue to `analyze_phase`.
</step>
...
**If `--auto` or `--all`** (per `modes/auto.md` or `modes/all.md`): Auto-select ALL gray areas. Log: `[--auto/--all] Selected all gray areas: [list area names].` Skip the Ask below and continue directly to `discuss_areas` with all areas selected.

**Otherwise, use Ask (multiSelect: true):**
- header: "Discuss"
...
- **--auto:** follow `workflows/discuss-phase/modes/auto.md` — Claude picks recommended option for every question; no Ask. Single-pass cap enforced.
...
- `--text` → `workflows/discuss-phase/modes/text.md` (replace Ask with plain-text numbered lists)
...
```
Apply each entry in `activeHooks` per @/home/bgshi/Development/python/correlia/.omp/gsd-core/references/loop-hook-dispatch.md. Empty list → continue to `confirm_creation`.
</step>
