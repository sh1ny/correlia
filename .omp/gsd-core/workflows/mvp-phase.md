...
<required_reading>
@/home/bgshi/Development/python/correlia/.omp/gsd-core/references/user-story-template.md
@/home/bgshi/Development/python/correlia/.omp/gsd-core/references/spidr-splitting.md
@/home/bgshi/Development/python/correlia/.omp/gsd-core/references/planner-mvp-mode.md
</required_reading>

<runtime_note>
**Copilot (VS Code):** Use `vscode_askquestions` wherever this workflow calls `Ask`. They are equivalent.

**TEXT_MODE fallback:** Set TEXT_MODE=true if `--text` is present in `{{GSD_ARGS}}` OR `text_mode` from init JSON is true. When TEXT_MODE is active, replace every Ask call with a plain-text numbered list and ask the user to type their choice number.
</runtime_note>
...
Normalize per `@/home/bgshi/Development/python/correlia/.omp/gsd-core/references/phase-argument-parsing.md` (zero-pad integer phases to two digits).
...
Use `Ask` with options [Re-prompt / Abort]. On Abort, exit cleanly. On Re-prompt, proceed.

## 3. User story prompts

Run three sequential `Ask` calls. Each is free-text. After all three, assemble into the canonical sentence per `@/home/bgshi/Development/python/correlia/.omp/gsd-core/references/user-story-template.md`:
...
Run the SPIDR rules from `@/home/bgshi/Development/python/correlia/.omp/gsd-core/references/spidr-splitting.md`. Briefly:
...
Use `Ask` with options [Yes, walk through SPIDR / No, proceed with the story as-is].
...
Use `Ask` with the five options from `spidr-splitting.md` (Spike / Paths / Interfaces / Data / Rules). Each option includes its targeted question as the description so the user can pick by understanding what each axis means.
...
Use `Ask` [Apply / Cancel]. On Cancel, exit without writing.
