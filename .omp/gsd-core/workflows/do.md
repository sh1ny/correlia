...
If `{{GSD_ARGS}}` is empty, ask via Ask:
...
**Ambiguity handling:** If the text could reasonably match multiple routes, ask the user via Ask with the top 2-3 options. For example:
...
If the chosen command expects a phase number and one wasn't provided in the text, extract it from context or ask via Ask.
