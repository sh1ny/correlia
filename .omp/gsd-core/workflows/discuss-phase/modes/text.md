# --text mode — plain-text overlay (no Ask)
...
When text mode is active, **do not use Ask at all**. Instead,
...
the equivalent Ask call.

This is required for OMP remote sessions (`/rc` mode) where the
