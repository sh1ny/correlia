...
**If `--auto` mode throughout:** Replace all Ask calls above with Claude's recommended choice. Log decisions inline. Apply the same logic as `--auto` in discuss-phase.

**Text mode (`workflow.text_mode: true` or `--text` flag):** Use plain-text numbered lists instead of Ask TUI menus.
...
ones). Reference: @/home/bgshi/Development/python/correlia/.omp/gsd-core/references/edge-probe.md.
...
# — NOT the consuming project's git root — falling back to git toplevel / /home/bgshi/Development/python/correlia/.omp.
...
EDGE_PROBE_JS=$(for _c in \
  ...
  "$_GSD_RT/.omp/bin/lib/edge-probe.cjs" \
  "/home/bgshi/Development/python/correlia/.omp/gsd-core/bin/lib/edge-probe.cjs" \
  "/home/bgshi/Development/python/correlia/.omp/bin/lib/edge-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)
...
If `$APPLICABLE` is `0`, do NOT proceed silently: ask the author to confirm via Ask
...
3. Resolve each with the user (Ask; text mode → numbered list):
...
requirement list). Reference: @/home/bgshi/Development/python/correlia/.omp/gsd-core/references/prohibition-probe.md — the
...
4. **Resolve each surfaced (non-canon) prohibition** (Ask; text mode → numbered list):
...
**Text mode (PROB-09):** per Step 5's text-mode rule, replace the Ask menus above
with plain-text numbered lists — there is NO hard Ask dependency, so the probe
...
Use the SPEC.md template from @/home/bgshi/Development/python/correlia/.omp/gsd-core/templates/spec.md.
