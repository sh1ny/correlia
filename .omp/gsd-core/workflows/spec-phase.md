...
ones). Reference: @.omp/gsd-core/references/edge-probe.md.
...
# — NOT the consuming project's git root — falling back to git toplevel / .omp.
...
EDGE_PROBE_JS=$(for _c in \
  ...
  ".omp/gsd-core/bin/lib/edge-probe.cjs" \
  ".omp/bin/lib/edge-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)
...
requirement list). Reference: @.omp/gsd-core/references/prohibition-probe.md — the
