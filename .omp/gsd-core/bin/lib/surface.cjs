...
function applySurface(runtimeConfigDir, layout, manifest, clusterMap) {
    ...
    for (const kind of layout.kinds) {
        ...
        try {
            ...
            if (staged && typeof kind.cleanup === 'function')
                kind.cleanup(staged);
        }
    }
    return resolved;
...
function _syncGsdDir(stagedDir, destDir, kind, manifest) {
    ...
    if (kindName === 'skills') {
        ...
        for (const entry of node_fs_1.default.readdirSync(destDir, { withFileTypes: true })) {
            if (!entry.isDirectory())
                continue;
            if (kindPrefix && !entry.name.startsWith(kindPrefix))
                continue;
            if (stagedDestNames.has(entry.name))
                continue;
            try {
        ...
        for (const file of node_fs_1.default.readdirSync(destDir).filter(f => f.endsWith('.md'))) {
            if (kindName === 'agents' && !file.startsWith('gsd-'))
                continue;
            if ((kindName === 'commands' || kindName === 'rules') && !namespacedByDir && kindPrefix && !file.startsWith(kindPrefix))
                continue;
