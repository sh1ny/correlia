...
<objective>
Manage the runtime skill surface without reinstall. Reads/writes `.omp/.gsd-surface.json`
(sibling to `.omp/.gsd-profile`) and re-stages the active skills directory in place.
Skill dirs live at `.omp/skills/gsd-*/`.
...
(`.omp`), NOT the skills sub-directory (`.omp/skills`).

This matches `installRuntimeArtifacts` and `uninstallRuntimeArtifacts`, which also
receive `.omp` as `configDir`. The skill dirs themselves live at
`.omp/skills/gsd-*/` because the `claude global` layout has `destSubpath =
'skills'` — they are derived from `configDir`, not the root for it.

```bash
# OMP — global install
RUNTIME_CONFIG_DIR="${CLAUDE_CONFIG_DIR:-.omp}"
SCOPE="global"
...
<execution_context>
Surface state file: `.omp/.gsd-surface.json`
Install profile marker: `.omp/.gsd-profile`
Skill dirs: `.omp/skills/gsd-*/`
Engine module: `.omp/gsd-core/bin/lib/surface.cjs`
Cluster definitions: `.omp/gsd-core/bin/lib/clusters.cjs`
</execution_context>
