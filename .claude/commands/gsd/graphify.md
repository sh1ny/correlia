...
**CJS-only (graphify):** `graphify` subcommands are not registered on `gsd-tools query`. Use `node .claude/gsd-core/bin/gsd-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `gsd-tools query` where a handler exists.
...
Knowledge graph is disabled. To activate:
  ...
  node .claude/gsd-core/bin/gsd-tools.cjs config-set graphify.enabled true
...
```bash
node .claude/gsd-core/bin/gsd-tools.cjs graphify query <term>
```
...
```bash
node .claude/gsd-core/bin/gsd-tools.cjs graphify status
```
...
```bash
node .claude/gsd-core/bin/gsd-tools.cjs graphify diff
```
...
```bash
node ".claude/gsd-core/bin/gsd-tools.cjs" graphify build
```
...
graphify update . \
  ...
  && node ".claude/gsd-core/bin/gsd-tools.cjs" graphify build snapshot \
  && node ".claude/gsd-core/bin/gsd-tools.cjs" graphify status
```
